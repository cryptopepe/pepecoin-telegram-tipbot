...
def deposit(bot, update):
	user = update.message.from_user.username
	if user is None:
		bot.send_message(chat_id=update.message.chat_id, text="You must set a telegram @username in your profile settings!")
	else:
		address = "./pepecoind"
	...
	if user is None:
		...
		if target == machine:
			...
			if balance > maxtip:
				...
				bot.send_message(chat_id=update.message.chat_id, text="@{0} tipped @{1} of {2} MEME / PEPE".format(user, target, amount))
		else: 
			bot.send_message(chat_id=update.message.chat_id, text="Error that user is not applicable.")

def balance(bot,update):
	quote_page = requests.get('https://www.worldcoinindex.com/coin/memetic')
	...
	price = float(price)bot.send_message(chat_id=update.message.chat_id, text="@{0} you have insufficent funds.".format(user))
	user = update.message.from_user.username
	if user is None:
		bot.send_message(chat_id=update.message.chat_id, text="Please set a telegram username in your profile settings!")
	else:
		...
		bot.send_message(chat_id=update.message.chat_id, text="@{0} your current balance is: {1} MEME / PEPE ≈  ${2}".format(user,balance,fiat_balance))
...
def price(bot,update):
	...
	quote_page = requests.get('https://bittrex.com/api/v1.1/public/getticker?market=btc-meme')
	soup = BeautifulSoup(quote_page.content, 'html.parser').text
	btc = soup[80:]
	sats = btc[:-2]
	bot.send_message(chat_id=update.message.chat_id, text="Memetic / PepeCoin is {0} Δ {1} ≈ {2}".format(price,percent,sats) + " ฿")
...
def marketcap(bot,update):
	...
	name = name_box.text.replace("\n","")
	mc = re.sub(r'\n\s*\n', r'\n\n', name.strip(), flags=re.M)
	bot.send_message(chat_id=update.message.chat_id, text="The current market cap of Memetic / PepeCoin is valued at {0}".format(mc))
